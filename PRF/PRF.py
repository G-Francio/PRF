...
class DecisionTreeClassifier:
    ...
    def node_arr_init(self):
        ...
        node_tree_results = numpy.ones([len(node_list), self.n_classes_
                                        ]) * (-1)
...
class RandomForestClassifier:
    ...
    def _choose_objects(self, X, pX, py, flags):
        ...
        objects_chosen = numpy.random.choice(objects_indices,
                                             nof_objects,
                                             replace=True)
    ...
    def _fit_single_tree(self, X, pX, py, flags):
        ...
        if self.bootstrap:
            X_chosen, pX_chosen, py_chosen, flags_chosen = self._choose_objects(
                X, pX, py, flags)
            tree.fit(X_chosen, pX_chosen, py_chosen, flags_chosen)
        else:
            tree.fit(X, pX, py, flags)
    ...
    def predict_single_object(self, row):
        ...
        for tree_index in range(0, self.n_estimators_):
            y_proba_tree = self.estimators_[
                tree_index].predict_proba_one_object(row)
    ...
    def predict_proba(self, X, dX=None, flags=None):
        ...
        # See here for the solution: https://stackoverflow.com/questions/34140560/accessing-and-altering-a-global-array-using-python-joblib

        def _predict_proba(_tree, _proba, _X=X, _dX=dX, _flags=flags):
            _tree.node_arr_init()
            self.predict_single_tree(_tree.predict_proba, _X, _dX, _flags,
                                     _proba)
        ...
        if self.ncpu is not None:
            ...
            proba = numpy.memmap(mmap_path,
                                 dtype=numpy.float64,
                                 shape=(X.shape[0], self.n_classes_),
                                 mode='w+')

            Parallel(n_jobs=self.ncpu)(
                delayed(_predict_proba)(tree, proba, X, dX, flags)
                for tree in self.estimators_)
        else:
            for i, tree in enumerate(self.estimators_):
                _predict_proba(tree, _proba=proba, _X=X, _dX=dX, _flags=flags)
    ...
    def apply(self, X, dX=None, flags=None):
        ...
        dX = numpy.zeros(
            X.shape)  # TODO: return leafs with probabilities for PRF

        for i, tree in enumerate(self.estimators_):
            tree.node_arr_init()

        leafs = [
            tree.predict_proba(X, dX, flags,
                               return_leafs=True)[:, 0].reshape(-1, 1)
            for tree in self.estimators_
        ]
    ...
    def predict(self, X, dX=None, flags=None, return_leafs=False):
        y_pred_inds = numpy.argmax(self.predict_proba(X, dX, flags), axis=1)
